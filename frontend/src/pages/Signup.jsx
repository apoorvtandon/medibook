...
const Signup = () => {
  ...
  const handleFileInputChange = async (event) => {
    ...
    setPreviewURL(data.url); // Assuming data.url is the URL of the uploaded image
    setSelectedFile(file); // Set the selected file itself, not data.url
...
const submitHandler = async (event) => {
  ...
  return (
    <section className='px-5 xl:px-0 mt-10 mb-20'>
      <div className='max-w-[1170px] mx-auto'>
        <div className='grid grid-cols-1 lg:grid-cols-2'>
          <div className='hidden lg:block bg-primaryColor rounded-l-lg'>
            <figure className='rounded-l-lg'>
              <img src={signupImg} alt="srf" className='w-full rounded-l-lg'/>
            </figure>
          </div> 
          <div className='rounded-l-lg lg:pl-16 py-10'>
            ...
            <input
              ...
              className='w-full pr-4 py-3 border-b border-solid border-[#0066ff61] focus:border-primaryColor text-[22px] leading-7 text-headingColor placeholder:text-textColor cursor-pointer focus:outline-none'
              required
            />
          </div>
          <div className='mb-5'>
  ...
  <option value="other">Other</option>
          ...
          <div className='mb-5 flex items-center gap-3'>
            ...
            <div className='relative w-[130px] h-[50px]'>
              <input type="file" name="photo" id="customFile" onChange={handleFileInputChange}  accept='image/*' className='absolute top-0 left-0 w-full h-full opacity-0 cursor-pointer'/>
          ...
          <div className='mt-7'>
            <button disabled={loading && true} type="submit" className='w-full bg-primaryColor text-white text-[18px] leading-[30px] rounded-lg px-4 py-3'>{loading? <HashLoader size={35}/>: 'Sign Up'}</button>
          </div>
